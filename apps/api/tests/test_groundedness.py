...
def test_apply_groundedness_policy_returns_answer_unchanged_when_failed() -> None:
    answer = "CASE-9999 is critical for Meridian."
    verification = GroundednessResult(
        passed=False,
        unsupported_claims=["Case reference 'CASE-9999' not found in tool outputs"],
...
def test_factual_without_tools_fails() -> None:
    result = verify_groundedness(
        "CASE-2001 is critical for Meridian Pay and SLA is breached.",
        [],
    )
    ...
    tool_calls = [
        {
            ...
            "result": {
                "ok": True,
                "open_issues": [{"issue_key": "CASE-2001", "priority": "critical"}],
            },
        }
    ]
    ok = verify_groundedness("CASE-2001 is critical.", tool_calls)
    assert ok.passed is True

    bad = verify_groundedness("CASE-9999 is critical.", tool_calls)
    ...
    tool_calls = [
        {
            ...
            "result": {
                "ok": True,
                "customer": {"external_id": "MERIDIAN", "name": "Meridian Pay"},
            },
        }
    ]
    ok = verify_groundedness("MERIDIAN is an enterprise account.", tool_calls)
    assert ok.passed is True

    bad = verify_groundedness("NORTHLINE is an enterprise account.", tool_calls)
    assert bad.passed is False
